...
export default function ItemReports() {
  ...
  useEffect(() => {
    if (userType === 'ADMIN' && commentId) {
      fetchReports(commentId as string);
    }
  }, [userType, commentId]);
  ...
  const fetchReports = async (id: string) => {
    ...
    try {
      const accessToken = localStorage.getItem('accessToken');
      const response = await fetch(`/api/reports/comment/${id}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        },
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-white mb-6">Reports for Comment {commentId}</h1>
